...
<body>
	...
	$stmt = $conn->prepare("SELECT * FROM videos WHERE name LIKE ? OR description LIKE ? LIMIT ?");
	$search_param = '%' . $search . '%';
	...
	if ($stmt->execute()) {
		...
		while ($row = $result->fetch_assoc()) {
			...
			$nnmme = $row["account_holder_name"];

			// Output search results
			?>
			<a href="<?= "../shorts-vidios.php?id=" . $row["id"] ?>" style="display: inline-block; width: 360px;">
				<div class="user-info" style="display: inline-block;">
					<div class="video-container">
						<video src="../<?= $video_file; ?>" style="width: 360px;" id="vidios"></video>
					</div>
